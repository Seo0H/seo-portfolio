...
- **구현 방식**
  ...
  > - `setTimeout`과 `useEffect CleanUp`을 이용해 디바인스를 구현하였습니다. [디바운스 구현 과정을 정리한 블로그 글](https://blog.seo0h.me/blog/study/debounce 'preview') / [디바운스 PR](https://github.com/SobiSa-Expense-Hunter/SobiSa/pull/195 'preview')
  >   <Captions>
...
- **구현 방식**
  ...
  >   링크](https://seo0h.notion.site/f8cd84534d99456b8d1e0bc3f4589caf?pvs=4 'preview'), [이전
  >   검색내역 보기 기획서 피그마
...
> **검색어 결과 무한스크롤로 표기**
>
> - Naver 쇼핑 api에서 제공하는 물건 리스트를 무한스크롤로 보여지도록 구현하였습니다.

> **커스텀 훅을 이용한 로직 분리**
>
> - 해당 로직을 뷰 로직에서 분리하여 Hook으로 분리했습니다.([useSearchProducts](https://github.com/SobiSa-Expense-Hunter/SobiSa/blob/develop/src/hooks/useSearchProduct.ts))
...
- **구현 방식**
  ...
  > - **동적으로 위치가 변하는 설명 애니메이션** > ![](/posts/sobisa/온보딩_오버레이.png)
