...
- `Indexed DB` 사용
  ...
  프로젝트에 Back-end가 없었기 때문에, 이전 검색했던 데이터를 다시 확인하기 위해서는 클라이언트에 데이터를 저장해야 했습니다. 기존 초기 방문 유저를 체크하기 위해 `LocalStorage` 를 사용하고 있었고, 상대적으로 큰 데이터를 저장해야 했기에 비동기식으로 작동하는 Indexed DB를 사용해 해당 기능을 구현하였습니다.ProjectMainImage.
